...
app.get('/dds/gestPha', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlPha = `SELECT * FROM profession WHERE role="6" AND wilayaId="${ddsWilaya}";`;
                db.query(sqlPha, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
                        ...
                        db.query(sqlWilayas, (err, wilayas) => {
                            if (err) {
                                ...
                                res.render('assets/dds/ddsGestPha', { reqs: reqs, rows: rows, wilayas: wilayas, erM: req.flash('erM'), suM: req.flash('suM') });
                            }
                        });
                    }
                });
...
app.post('/dds/gestPha/addPha', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
            ...
            db.query(sqlCheck, (err, check) => {
                if (err) {
                    ...
                    if (check.length == 1) {
                        ...
                        let sqlPha = `INSERT INTO profession(matricule,wilayaId,role,nom,prenom,numTelephone,dateDeNaissance,dateDeRegistre,emailDDS) VALUES("${matricule}","${wilayaId}","${role}","${nom}","${prenom}","${numTelephone}","${dateDeNaissance}","${dateDeRegistre.getFullYear()}-${dateDeRegistre.getMonth() + 1}-${dateDeRegistre.getDate()}","${emailDDS}");`;
                        db.query(sqlPha, (err, result) => {
                            if (err) {
                                throw err;
                            } else {
...
app.get('/dds/gestPha/getPha', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId
        let sqlReq = `SELECT * FROM demandes WHERE wilayaId=${ddsWilaya} ORDER BY dateDeDemande`;
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlPha = `SELECT * FROM profession WHERE wilayaId=${ddsWilaya} AND role="6" AND matricule LIKE "${dynamicMatricule}" AND nom LIKE "${dynamicNom}" AND prenom LIKE "${dynamicPrenom}";`;
                db.query(sqlPha, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
...
app.get('/dds/gestPha/profilePha/:matricule', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlPha = `SELECT profession.matricule AS matricule,profession.nom,profession.prenom,profession.numTelephone,profession.dateDeNaissance,profession.dateDeRegistre,photoprofession.photoPathProfession,accountproffesion.email,accountproffesion.password,profession.role
            FROM profession
            ...
            WHERE profession.wilayaId="${ddsWilaya}" AND profession.matricule="${matricule}";`;
                db.query(sqlPha, (err, rows) => {
                    if (err) {
                        ...
                        res.render('assets/dds/ddsProfilePha', { reqs: reqs, rows: rows });
                    }
                });
...
app.post('/dds/gestPha/profilePha/:matricule/editAcc',auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
            ...
            if (err) {
                ...
                db.query(sqleditAcc, (err, acc) => {
                    if (err) {
                        ...
                        res.redirect(`/dds/gestPha/profilePha/${matricule}`);
                    }
                });
            }
        });
...
app.get('/dds/gestPha/profilePha/:matricule/deleteProfile', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
