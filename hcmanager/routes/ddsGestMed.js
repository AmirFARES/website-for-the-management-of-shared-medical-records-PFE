...
app.get('/dds/gestMed', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlMed = `SELECT * FROM profession LEFT JOIN medecinInfo ON profession.matricule=medecinInfo.matricule WHERE role="1" AND wilayaId="${ddsWilaya}";`;
                db.query(sqlMed, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
                        ...
                        db.query(sqlWilayas, (err, wilayas) => {
                            if (err) {
                                ...
                                res.render('assets/dds/ddsGestMed', { reqs: reqs, rows: rows, wilayas: wilayas, erM: req.flash('erM'), suM: req.flash('suM') });
                            }
                        });
                    }
                });
...
app.post('/dds/gestMed/addMed', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
            ...
            db.query(sqlCheck, (err, check) => {
                if (err) {
                    ...
                    if (check.length == 1) {
                        ...
                        let sqlMed = `INSERT INTO profession(matricule,wilayaId,role,nom,prenom,numTelephone,dateDeNaissance,dateDeRegistre,emailDDS) VALUES("${matricule}","${wilayaId}","${role}","${nom}","${prenom}","${numTelephone}","${dateDeNaissance}","${dateDeRegistre.getFullYear()}-${dateDeRegistre.getMonth() + 1}-${dateDeRegistre.getDate()}","${emailDDS}");`;
                        db.query(sqlMed, (err, result) => {
                            if (err) {
                                throw err;
                            } else {
                                let sqlMedInfo = `INSERT INTO medecinInfo(etat,speciality,matricule) VALUES("${etat}","${speciality}","${matricule}");`;
                                db.query(sqlMedInfo, (err, result) => {
                                    if (err) {
                                        throw err;
                                    } else {
...
app.get('/dds/gestMed/getMed', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId
        let sqlReq = `SELECT * FROM demandes WHERE wilayaId=${ddsWilaya} ORDER BY dateDeDemande`;
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlMed = `SELECT * FROM profession LEFT JOIN medecinInfo ON profession.matricule=medecinInfo.matricule WHERE wilayaId=${ddsWilaya} AND role="1" AND profession.matricule LIKE "${dynamicMatricule}" AND nom LIKE "${dynamicNom}" AND prenom LIKE "${dynamicPrenom}";`;
                db.query(sqlMed, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
...
app.get('/dds/gestMed/profileMed/:matricule', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId;
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlMed = `SELECT profession.matricule AS matricule,profession.nom,profession.prenom,profession.numTelephone,profession.dateDeNaissance,profession.dateDeRegistre,photoprofession.photoPathProfession,accountproffesion.email,accountproffesion.password,medecininfo.etat,medecininfo.speciality,profession.role
            FROM profession
            ...
            LEFT JOIN medecinInfo ON profession.matricule=medecinInfo.matricule
            WHERE profession.wilayaId="${ddsWilaya}" AND profession.matricule="${matricule}";`;
                db.query(sqlMed, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
                        res.render('assets/dds/ddsProfileMed', { reqs: reqs, rows: rows });
                    }
                });
...
app.post('/dds/gestMed/profileMed/:matricule/editAcc',auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
            ...
            if (err) {
                ...
                db.query(sqleditAcc, (err, acc) => {
                    if (err) {
                        ...
                        res.redirect(`/dds/gestMed/profileMed/${matricule}`);
                    }
                });
            }
        });
...
app.get('/dds/gestMed/profileMed/:matricule/deleteProfile', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
        ...
        db.query(sqlPhoto, (err, result) => {
            if (err) {
                ...
                let sqlInfo = `DELETE FROM medecinInfo WHERE matricule="${matricule}";`;
