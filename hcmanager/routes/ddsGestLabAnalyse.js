...
app.get('/dds/gestLabAnalyse', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlLabAnalyse = `SELECT * FROM profession WHERE role="4" AND wilayaId="${ddsWilaya}";`;
                db.query(sqlLabAnalyse, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
                        ...
                        db.query(sqlWilayas, (err, wilayas) => {
                            if (err) {
                                ...
                                res.render('assets/dds/ddsGestLabAnalyse', { reqs: reqs, rows: rows, wilayas: wilayas,erM:req.flash('erM'),suM:req.flash('suM') });
                            }
                        });
                    }
                });
...
app.post('/dds/gestLabAnalyse/addLabAnalyse', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        ...
        if(matricule.length==0||wilayaId.length==0||nom.length==0||prenom.length==0||numTelephone.length==0||dateDeNaissance.length==0){
            ...
            res.redirect('/dds/gestLabAnalyse');
        }else{
            ...
            db.query(sqlCheck, (err, check) => {
                if (err) {
                    ...
                    if (check.length==1) {
                        ...
                        let sqlLabAnalyse = `INSERT INTO profession(matricule,wilayaId,role,nom,prenom,numTelephone,dateDeNaissance,dateDeRegistre,emailDDS) VALUES("${matricule}","${wilayaId}","${role}","${nom}","${prenom}","${numTelephone}","${dateDeNaissance}","${dateDeRegistre.getFullYear()}-${dateDeRegistre.getMonth() + 1}-${dateDeRegistre.getDate()}","${emailDDS}");`;
                        db.query(sqlLabAnalyse, (err, result) => {
                            if (err) {
                                throw err;
                            } else {
...
app.get('/dds/gestLabAnalyse/getLabAnalyse', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId
        let sqlReq = `SELECT * FROM demandes WHERE wilayaId=${ddsWilaya} ORDER BY dateDeDemande`;
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlLabAnalyse = `SELECT * FROM profession WHERE wilayaId=${ddsWilaya} AND role="4" AND matricule LIKE "${dynamicMatricule}" AND nom LIKE "${dynamicNom}" AND prenom LIKE "${dynamicPrenom}";`;
                db.query(sqlLabAnalyse, (err, rows) => {
                    if (err) {
                        throw err;
                    } else {
...
app.get('/dds/gestLabAnalyse/profileLabAnalyse/:matricule', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let ddsWilaya = req.ddsUser.wilayaId
        ...
        db.query(sqlReq, (err, reqs) => {
            if (err) {
                ...
                let sqlLabAnalyse = `SELECT profession.matricule AS matricule,profession.nom,profession.prenom,profession.numTelephone,profession.dateDeNaissance,profession.dateDeRegistre,photoprofession.photoPathProfession,accountproffesion.email,accountproffesion.password,profession.role
            FROM profession
            ...
            WHERE profession.wilayaId="${ddsWilaya}" AND profession.matricule="${matricule}";`;
                db.query(sqlLabAnalyse, (err, rows) => {
                    if (err) {
                        ...
                        res.render('assets/dds/ddsProfileLabAnalyse', { reqs: reqs, rows: rows });
                    }
                });
...
app.post('/dds/gestLabAnalyse/profileLabAnalyse/:matricule/editAcc', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
            ...
            if (err) {
                ...
                db.query(sqleditAcc, (err, acc) => {
                    if (err) {
                        ...
                        res.redirect(`/dds/gestLabAnalyse/profileLabAnalyse/${matricule}`);
                    }
                });
            }
        });
...
app.get('/dds/gestLabAnalyse/profileLabAnalyse/:matricule/deleteProfile', auth.isLoggedIn, function (req, res) {
    if (req.ddsUser) {
        let matricule = req.params.matricule;
